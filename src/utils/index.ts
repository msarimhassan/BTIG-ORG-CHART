...
export const checkKey = (key: any, user: any) => {
  if (key === 'displayName')
    ...
    return Urls.updateHorizontalDimension(
      user.userPrinicipalName,
      user.horizontal
    )
  else if (key === 'visible')
    return Urls.updateVisible(user.userPrinicipalName, user.visible)
  else if (key === 'left')
    return Urls.updateLeftDimension(user.userPrinicipalName, user.left)
  else if (key === 'Team')
